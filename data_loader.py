...
class Utterances(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        timbre_features = extract_timbre_features(wav_mono, 16000)
        ...
        return wav_mono, spk_id_org, spmel, rhythm_input, content_input, pitch_input, timbre_input, timbre_features
...
class Collator(object):
    ...
    def __call__(self, batch):
        new_batch = []
        for token in batch:
            _, spk_id_org, spmel_gt, rhythm_input, content_input, pitch_input, timbre_input, timbre_features = token
            ...
            timbre_features = timbre_features[:, left:left + len_crop]  # [80, Lc]
            ...
            pitch_input = np.pad(pitch_input[:, np.newaxis], ((0, self.max_len_pad - pitch_input.shape[0]), (0, 0)),
                                 'constant', constant_values=-1e10)
            timbre_features = np.pad(timbre_features, ((0, 0), (0, self.max_len_pad - timbre_features.shape[1])), 'constant')

            new_batch.append((spk_id_org, spmel_gt, rhythm_input, content_input, pitch_input, timbre_input, len_crop, timbre_features))

        batch = new_batch
        spk_id_org, spmel_gt, rhythm_input, content_input, pitch_input, timbre_input, len_crop, timbre_features = zip(*batch)
        ...
        timbre_features = torch.FloatTensor(np.stack(timbre_features, axis=0)).permute(0, 2, 1)  # [16, 192, 80]

        return spk_id_org, spmel_gt, rhythm_input, content_input, pitch_input, timbre_input, len_crop, timbre_features
